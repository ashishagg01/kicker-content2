...
public class BreadcrumbImpl extends WCMUse {

	// pageList is an ArrayList used to pass on the required data to
	// breadcrumb.html
	...
	public void activate() throws Exception {
		...
		// By default breadcrumbs will start from the level 2-> requirement
		// specs.
		...
		/*
		 * The Breadcrumbs will start from the level 2 under locale on the left
		 * ends with the current page on the right. The parent level is reduced
		 * by 1 with each loop and the respective parent is stored in a list.
		 */
		while (absParentLevel < currentLevel - relParentLevel + 1) {
			...
			// In case the the current Page has no parent, break. As there is
			// nothing to return/display.
			if (trail == null) {
				break;
			...
			/*
			 * If the Navigation Title entered in page properties then it will
			 * be passed, else if the Page title exists, it will be picked. else
			 * the page name will be fetched.
			 */
